...
func TestRecordDelete(t *testing.T) {
	...
	*/
	// PostgreSQL:
	expectedRelManyPart := `SELECT "demo1".* FROM "demo1" WHERE EXISTS (SELECT 1 FROM json_array_elements_text(CASE WHEN [[demo1.rel_many]] IS JSON OR json_valid([[demo1.rel_many]]::text) THEN [[demo1.rel_many]]::json ELSE json_array([[demo1.rel_many]]) END) {{__je__}} WHERE [[__je__.value]]::text='`
